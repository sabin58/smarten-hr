...
export class TimeOffCalendarModel extends NepaliCalendarModel {
  ...
  makeContextDefaults(record) {
    ...
    let defaultEmployeeId = this.employeeId
    if (context.active_model === "hr.employee") {
      defaultEmployeeId = context.active_id
    }
    ...
    if (["week", "day"].includes(this.scale)) {
      ...
      const hourFrom = deserialize(context.default_date_from ?? this.date)
      const hourTo = deserialize(context.default_date_to ?? this.date)
      context.default_request_hour_from = hourFrom.hour + hourFrom.minute / 60
      context.default_request_hour_to = hourTo.hour + hourTo.minute / 60
    }
    ...
    return this.orm.searchRead(
      ...
      [
        ...new Set([
          ...fieldNames,
          ...fieldNamesToAdd,
          ...Object.keys(this.meta.activeFields),
        ]),
      ],
      { context }
    )
